...
class ReorderableListViewController: UIViewController {
    ...
    #else
    private(set) lazy var collectionView = CollectionView<Section, Item>(layout: createLayout())
    #endif
...
extension ReorderableListViewController {
    ...
    func configureDataSource() {
        ...
        collectionView.dataManager.diffableDataSource.reorderingHandlers.canReorderItem = { item in return true }
        collectionView.dataManager.diffableDataSource.reorderingHandlers.didReorder = { [weak self] transaction in
            guard let self = self else { return }
    ...
    func applyInitialBackingStore(animated: Bool = false) {
        ...
        #else
        for (section, items) in initialBackingStore() {
            collectionView.dataManager.applyItems(items, updatedSection: section)
        }
        #endif
    }
    ...
    func applySnapshotsFromBackingStore(animated: Bool = false) {
        ...
        #else
        collectionView.dataManager.applySections(backingStore.map {
            ($0.key, $0.value)
        })
        #endif
    }
